    ...
    <div className='flex justify-center items-center py-12 pt-36 flex-col'>
      ...
      {Ourproducts.products.length >= 1 ? (
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {Ourproducts.products.map((product) => {
            const [hovering, setHovering] = React.useState(false);
            return (
              <div
                key={product.id}
                className="w-full relative rounded-3xl overflow-hidden max-w-md mx-auto bg-gradient-to-r from-[#1D2235] to-[#121318] p-4 "
              >
                <Rays />
                <Beams />
                <div className="relative z-10 flex flex-col items-center justify-center">
                  <Lens hovering={hovering} setHovering={setHovering}>
                    <Image
                      src={product.image} 
                      alt={product.title || "Product Image"} // Fallback for accessibility
                      width={300}
                      height={300}
                  ...
                  >
                    ...
                    >
                      <button
                        className="text-white text-sm text-left bg-black border-2 border-black my-4 rounded-full p-3 hover:border-gray-400 transition duration-500"
                      >
                        {product.title || "Product Name"}
                      </button>
        ...
        <h2 className="text-3xl font-serif">
          Our fascinating products are on the way.
        </h2>
      )}
